...
$pdf->SetTitle('LISTADO DE APODERADOS');
...
$pdf->Cell(100, 6, 'LISTA DE APODERADOS', 0, 0, 'C');
...
$pdf->Cell(115, 6, 'Accionista', 1, 0, 'C', 1);
$pdf->Cell(20, 6, 'CcApoder.', 1, 0, 'C', 1);
//$pdf->Cell(50,6,'Apoderado',1,0,'C',1); 
$pdf->Cell(8, 6, 'Acc', 1, 0, 'C', 1);
...
$sqlReportesAccionistas = ("SELECT * FROM poderes  ORDER BY ID ASC");
//$sqlTrabajadores = ("SELECT * FROM trabajadores");
$query = mysqli_query($conexion, $sqlReportesAccionistas);

while ($dataRow = mysqli_fetch_array($query)) {
    $pdf->Cell(10, 6, ($dataRow['ID']), 1, 0, 'C');
    $pdf->Cell(20, 6, ($dataRow['ccAccionista']), 1, 0, 'C');
    $pdf->Cell(115, 6, ($dataRow['nmAccionista']), 1, 0, 'C');
    $pdf->Cell(20, 6, ($dataRow['ccApoderado']), 1, 0, 'C');
    //$pdf->Cell(50,6,$dataRow['nmApoderado'],1,0,'C');
    $pdf->Cell(8, 6, ($dataRow['nAcciones']), 1, 0, 'C');
    $pdf->Cell(20, 6, (date('m-d-Y', strtotime($dataRow['fecha']))), 1, 1, 'C');
}


$pdf->AddPage(); //Agregar nueva Pagina

$pdf->Output('Reporte Accionistas ' . date('d_m_y H :i :s') . '.pdf', 'D');
?>
